...
test('两个目录相关的中文文案都配了英文', () => {
  ...
  for (const key of [
    ...
    '旧目录里有 {count} 项插件和 profile 内容。搬过去后旧目录就空了；插件里的链接带绝对路径，搬完若报解析不到 bundle，启动时会自动重装依赖。',
    '读不到旧目录里有多少东西，先按不迁移处理。',
    '版本目录已改为 {dir}，{count} 个已装版本一起搬过去了。',
    '版本目录已改为 {dir}（旧目录里没有已装版本，没什么可搬）。',
    '版本目录已改为 {dir}；旧目录里的版本没有搬，需要就手动复制过去。',
    '插件目录已改为 {dir}，{count} 项内容一起搬过去了。',
    '插件目录已改为 {dir}（旧目录里没有要搬的内容）。',
    '插件目录已改为 {dir}；旧目录里的插件内容没有搬。',
    '插件、profile 和 dsh 的数据都在这里；改完要重启 dsh 才生效。默认 {home}。',
    'dsh 各版本装在这里（几百 MB 一个）。',
    '重启启动器后管理页改到 {port} 端口。',
    '当前实际监听 {live} 端口（配置的 {port} 被别的程序占用，启动时自动顺延）。',
  ]) {
    assert.ok(dictionary.includes(`"${key}": `), `缺英文词条：${key}`)
  }
  // 旧文案（"不会自动迁移"、拼在整句里的"已保存。"前缀）不该留在字典里当僵尸
  assert.ok(!dictionary.includes('已安装的版本不会自动迁移'), '旧措辞要清掉')
  assert.ok(!dictionary.includes('"已保存。版本目录'), '回执句子拆开后不该还带着"已保存。"前缀')
})
